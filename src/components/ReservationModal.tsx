...
const ReservationModal: React.FC<ReservationModalProps> = ({
  ...
  const validateAndAdvanceToNextStep = () => {
    for (let index = 0; index < steps.length - 1; index++) {
      const valid = isStepValid(index);
      console.log(valid, index, "checkStep");

      if (!valid) {
        setCurrentStep(index); // ❌ Invalid step, go there and stop
        return;
      }
    }

    // ✅ All steps are valid up to the current one, go to the next step
  ...
  const handleVoiceProcessed = (result: ProcessedVoiceResponse): void => {
    ...
    if (onAIMessage) {
      ...
      onAIMessage(
        `User: "${voiceResult.originalInput || "Voice input received"}"`,
        false
      );
  ...
  const handleSubmit = async () => {
    ...
    setTimeout(() => {
      ...
      const confirmationData = {
        confirmationNumber: `LG${Math.random()
          .toString(36)
          .substr(2, 8)
          .toUpperCase()}`,
        guestName: formData.guestName,
  ...
  const getFieldSx = (field: string) => ({
    "& .MuiOutlinedInput-root": {
      backgroundColor: isVoiceFilled(field)
        ? "success.light"
        : "background.paper",
      "& fieldset": {
              ...
              <Grid container spacing={3}>
                <Grid size={{ xs: 12, sm: 6 }}>
                  <DatePicker
                    ...
                    onChange={(newValue) =>
                      setFormData({ ...formData, checkIn: newValue })
                    }
                    slotProps={{
                      textField: {
                        fullWidth: true,
                        ...
                        helperText: isVoiceFilled("checkIn")
                          ? "✓ Filled by voice"
                          : undefined,
                      },
                    }}
                ...
                <Grid size={{ xs: 12, sm: 6 }}>
                  <DatePicker
                    ...
                    onChange={(newValue) =>
                      setFormData({ ...formData, checkOut: newValue })
                    }
                    slotProps={{
                      textField: {
                        fullWidth: true,
                        ...
                        helperText: isVoiceFilled("checkOut")
                          ? "✓ Filled by voice"
                          : undefined,
                      },
                    }}
